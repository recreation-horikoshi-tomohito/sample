...
def test_在籍中社員のみが返される(app, db_session):
    seed(db_session, [
        {"name": "在籍中社員", "role": "エンジニア", "position": "一般",
         "department": "開発部", "age": 30, "hire_date": "2022-04-01", "status": "在籍中"},
        {"name": "退職済社員", "role": "デザイナー", "position": "一般",
         "department": "デザイン部", "age": 35, "hire_date": "2018-04-01", "status": "退職済"},
    ])
    with app.app_context():
        result = FindEmployeeListUseCase(SQLiteEmployeeRepository()).execute()
    assert len(result) == 1
